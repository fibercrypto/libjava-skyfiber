...
public class transutils extends skycoin {
  ...
  // { cipher_PubKey p = new cipher_PubKey(); SKY_cipher_GenerateKeyPair(p,
  // secKey); GoSlice b = new GoSlice(); SKY_cipher_RandByte(128, b);
  // cipher_SHA256 h = new cipher_SHA256();
  ...
  // uxBody.setAddress(a);
  // uxBody.setCoins(BigDecimal.valueOf(1e6).toBigInteger());
  // uxBody.setHours(BigDecimal.valueOf(100).toBigInteger());
  // }

  // public void makeUxOutWithSecret(coin__UxOut uxOut, cipher_SecKey secKey) {
  // coin__UxBody uxBody = new coin__UxBody();
  // makeUxBodyWithSecret(uxBody, secKey);
  // uxOut = new coin__UxOut();
  // coin__UxHead uxHead = new coin__UxHead();
  // uxHead.setTime(BigDecimal.valueOf(100).toBigInteger());
  // uxHead.setBkSeq(BigDecimal.valueOf(2).toBigInteger());
  // uxOut.setBody(uxBody);
