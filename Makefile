...
build-libjava:
	...
	cp $(PWD)/build/usr/lib/libskycoin.so ~/Library/
	sudo cp $(PWD)/build/usr/lib/libskycoin.so /Library/

test: build-libc build-swig build-libjava
	mvn clean
	$(LDPATHVAR)="$(PWD)/build/usr/lib/:$(LDPATHVAR):$(HOME)/Library/:/Library/" mvn test
	mvn clean